...
class VillageState {
    ...
    move(destination) {
        if (!roadGraph[this.place].includes(destination)) {
            ...
            let parsels = this.parcels.map(p => {
                if (p.place !== this.place) {return p};
                return {place: destination, address: p.address};
            }).filter(p => p.place !== p.address);
            return new VillageState(destination, parsels);
        }
    }
}
...
//console.log(first.parsels);//[ { place: 'Post Office', address: "Alice's House" } ]
